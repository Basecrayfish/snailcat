...
src_install() {
	...
	cp "${FILESDIR}"/bsd.lib.mk "${ED}"/usr/share/mk/${PN} || die
	cp "${FILESDIR}"/files.mk "${ED}"/usr/share/mk/${PN} || die
	cp "${FILESDIR}"/inc.mk "${ED}"/usr/share/mk/${PN} || die
	cp "${FILESDIR}"/scripts.mk "${ED}"/usr/share/mk/${PN} || die
}

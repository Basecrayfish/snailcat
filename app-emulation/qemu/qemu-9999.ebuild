...
src_compile() {
	if [[ -n ${user_targets} ]]; then
		touch ${T}/user-targets-exist
	fi

	if [[ -e ${T}/user-targets-exist ]]; then
		cd "${S}/user-build"
		default
	fi

	if [[ -n ${softmmu_targets} ]]; then
		touch ${T}/softmmu-targets-exist
	fi

	if [[ -e ${T}/softmmu-targets-exist ]]; then
		cd "${S}/softmmu-build"
		default
	fi
...
pkg_postinst() {
	if [[ -e ${T}/softmmu-targets-exist ]] && use kernel_linux; then
		udev_reload
	fi

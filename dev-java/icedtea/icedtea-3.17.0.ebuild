...
src_configure() {
	...
	# My patches
	ln -s "${FILESDIR}/${PN}-3.17.0-jdk-clang-autoconf.patch" patches || die
	ln -s "${FILESDIR}/${PN}-3.17.0-jdk-clang-autogen.patch" patches || die
	ln -s "${FILESDIR}/${PN}-3.17.0-jdk-clang-jsig.patch" patches || die
	ln -s "${FILESDIR}/${PN}-3.17.0-jdk-libcxx.patch" patches || die
	ln -s "${FILESDIR}/${PN}-3.17.0-jdk-musl-isnanf.patch" patches || die

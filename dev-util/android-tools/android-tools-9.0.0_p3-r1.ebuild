...
src_prepare() {
	...
	eapply "${FILESDIR}"/${PN}-9.0.0_p3-core-musl-glibcisms.patch
	eapply "${DISTDIR}/${GLIBC_GETTID_PATCH}"
	sed -e 's|^#include <sys/cdefs.h>$|/*\0*/|' \
		-e 's|^__BEGIN_DECLS$|#ifdef __cplusplus\nextern "C" {\n#endif|' \
		-e 's|^__END_DECLS$|#ifdef __cplusplus\n}\n#endif|' \
		-i libcutils/include/{cutils/trace,private/fs_config,private/canned_fs_config}.h liblog/include/log/log_main.h liblog/log_portability.h \
			libziparchive/include/ziparchive/zip_archive.h || die
	...
	eapply "${FILESDIR}"/${PN}-9.0.0_p3-e2fsprogs-musl-types.patch

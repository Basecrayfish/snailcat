...
IUSE="clippy cpu_flags_x86_sse2 debug doc libressl llvm-libunwind miri nightly parallel-compiler rls rustfmt system-bootstrap system-llvm test wasm ${ALL_LLVM_TARGETS[*]}"
...
PATCHES=(
	...
	# "${FILESDIR}"/1.48.0-gentoo-musl-target-specs.patch # Breaks firefox
...
src_prepare() {
	...
	if use llvm-libunwind; then
		eapply "${FILESDIR}"/1.47.0-musl-llvm-libunwind.patch
	fi

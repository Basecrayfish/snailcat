...
inherit toolchain-funcs elisp-common flag-o-matic l10n perl-module bash-completion-r1 python-single-r1 systemd
...
LICENSE="GPL-2"
SLOT="0"
IUSE="+blksha1 +curl cgi doc emacs gnome-keyring +gpg highlight +iconv libressl mediawiki mediawiki-experimental +nls +pcre +pcre-jit perforce +perl pie +ppcsha1 static tk +threads +webdav xinetd cvs subversion test"
...
# Gnome-keyring's libsecret doesn't have static-libs flag
STATIC_DEPEND="
	!libressl? ( dev-libs/openssl:0=[static-libs] )
	libressl? ( dev-libs/libressl:=[static-libs] )
	sys-libs/zlib[static-libs(+)]
	pcre? (
		pcre-jit? ( dev-libs/libpcre2[jit(+),static-libs(+)] )
		!pcre-jit? ( dev-libs/libpcre[static-libs(+)] )
	)
	curl? (
		net-misc/curl[static-libs(+)]
		webdav? ( dev-libs/expat[static-libs(+)] )
	)
...
BDEPEND="static? ( curl? (virtual/pkgconfig) )"
...
exportmakeopts() {
	...
	sed -i -e '/\/usr\/local/s/BASIC_/#BASIC_/' \
		-e 's/^EXTLIBS =$/EXTLIBS = $(OPTEXTLIBS)/' \
		Makefile || die
	...
	if use static; then
		use curl && extlibs+=( $(pkgconf --with-path=${ESYSROOT}/usr/$(get_libdir)/pkgconfig --static --libs libcurl) )
	fi
...
src_prepare() {
	...
	# TODO replace EXTLIBS with EXTLIBS = OPTEXLIBS and pass as make arg
	sed -i \

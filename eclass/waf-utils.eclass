...
# @ECLASS-VARIABLE: WAF_LIBDIR_SUFFIX
# @DESCRIPTION:
# Set this to install libraries into a subdirectory in ${EPREFIX}/usr/$(get_libdir)
# Added to prevent certain libraries appearing PATH that are still needed to build other projects
# Only usage so far is for installing the jack libs for pipewire's jack emulation
: ${WAF_LIBDIR_SUFFIX:=""}
...
waf-utils_src_configure() {
	...
	if [[ ${waf_help} == *--libdir* ]]; then
		conf_args+=( --libdir="${EPREFIX}/usr/$(get_libdir)/${WAF_LIBDIR_SUFFIX}" )
	fi
